...
if(isset($_SESSION['cart']))
{
  ...
  $check_avail = 0;
  $check_avail = check_duplicate_product($companyname,$productname,$unit,$packsize);
  $available_qty=0;
  $check_qty = 0;

  if($check_avail==0)
  {
    $available_qty=check_stock_qty($companyname,$productname,$unit,$packsize,$link);
    if($available_qty>=$qty)
    {
      $b =array("companyname"=>$companyname,"productname"=>$productname,"unit"=>$unit,"packingsize"=>$packsize,"price"=>$price,"qty"=>$qty);
  ...
  else
  {
    $av_qty=0;
    $exist_qty=0;
    $exist_qty=check_sess_qty($companyname,$productname,$unit,$packsize,$link);
...
else
{
  ...
  if($available_qty>=$qty)
  {
    $_SESSION['cart'] = array(array("companyname"=>$companyname,"productname"=>$productname,"unit"=>$unit,"packingsize"=>$packsize,"price"=>$price,"qty"=>$qty));
  }
  else
  {
    echo "Entered Quantity is not available";
